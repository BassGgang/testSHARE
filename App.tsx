    ...
    if (!IS_TEST_MODE) {
      ...
      if (currentStep === 'SUBMITTER') {
        if (data.departmentRole === '共催 その他' && !data.otherDepartmentName?.trim()) {
          showError('主催部局を入力してください。');
          return;
        }
        if (!data.deanApproval) {
          showError('部局長の承認確認が必要です。');
          return;
        }
      }
      ...
      if (currentStep === 'ASSETS') {
        if (route === 'A' && !data.youtubeCopyrightConfirm) {
          showError('「動画公開についての権利・編集の確認」の同意チェックを入れてください。');
          return;
        }
      }
      ...
      if (currentStep === 'POLICIES') {
        if (!data.materialCopyrightConfirm || !data.fourWeekConsent || !data.speakerInstructionConfirm) {
          showError('必須の確認事項にチェックを入れてください。');
          return;
        }
        if (data.hasQA) {
          if (!data.qaParticipantConsent) {
            showError('参加者の顔・声の公開について選択してください。');
            return;
          }
          if (!data.qaMethod) {
            showError('質疑応答部分の公開方法について選択してください。');
            return;
          }
        }
        if (data.hasInterpretation) {
          if (!data.interpreterConsentRadio) {
            showError('同時通訳・手話通訳の同意について選択してください。');
            return;
          }
        }
      }
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-10 lg:p-14">
            <AnimatePresence>
              {errorMsg && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="mb-8 p-4 bg-red-50 border border-red-200 text-red-700 rounded-2xl flex items-start space-x-3 text-sm font-bold"
                >
                  ...
                  <button 
                    ...
                    className="text-red-400 hover:text-red-650 font-normal text-xs"
                  >
                    閉じる
                  </button>
                </motion.div>
                        ...
                        <div className="space-y-6">
                          <div className="space-y-2">
                            <label className="text-sm font-bold text-slate-700 flex items-center">
                              YouTube URL
                              <span className="text-[11px] font-normal text-slate-400 ml-2">（任意 - 既にYouTubeで動画を公開済みの場合のみ入力してください）</span>
                            </label>
                            <div className="relative">
                          ...
                          <div className="space-y-2 bg-amber-50 p-5 rounded-2xl border border-amber-200">
                            <h4 className="text-sm font-bold text-amber-900 mb-2">動画公開についての権利・編集の確認</h4>
                            <label className="flex items-start space-x-3 cursor-pointer group">
                              <div className="relative flex items-center h-5 mt-0.5">
                                <input 
                                  type="checkbox" 
                              ...
                              <div className="flex flex-col">
                                <span className="text-sm font-bold text-amber-900 leading-relaxed group-hover:text-amber-805 transition-colors">
                                  著作権及び肖像権などの権利処理・映像編集が完了している動画を公開することに同意します。 <RequiredBadge />
                                </span>
                                <p className="text-[11px] text-amber-700 mt-1">※完成動画の公開にあたり、必須のご確認事項となります（YouTube以外の動画ファイル提供の場合も必須です）。</p>
                              </div>
                            </label>
                          </div>
                        </div>
            ...
            <div className="mt-12 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-slate-100 pt-10">
              ...
              <div className="w-full md:w-auto flex flex-col md:flex-row gap-4 items-center">
                ...
                {currentStep === 'CONFIRM' ? (
                  <button key="submit-btn" type="submit" className="w-full md:w-auto px-12 py-4 bg-green-600 text-white rounded-2xl font-black text-lg hover:bg-green-700 transition-all shadow-xl shadow-green-100 active:scale-95">
                    申請を送信する
                  </button>
                ) : (
                  <button key="next-btn" type="button" onClick={nextStep} className="w-full md:w-auto flex items-center justify-center space-x-2 px-12 py-4 bg-blue-600 text-white rounded-2xl font-black text-lg hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 font-sans">
